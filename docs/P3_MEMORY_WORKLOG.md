...
拍内**不再发生 LLM/TTS**；落库归属见下方 D2 裁决（**留在心跳**，不移入 worker）。

**D2 已拍板＝记忆写入留在心跳**（2026-09-24，用户裁定，与我原建议 (b) 相反）。职责切分：

- **worker 只做耗时那段**：构造指令（`build_expression` + `enrich_permits` + 记忆/身份注入）
  → `await llm.speak` → `await tts.speak`
- **全部落库留在心跳**：`think` / `expression` / `add_memory` 仍由心跳写；
  `user_message → 经历`（`heartbeat.py` L253-279）**一字不动**
- **回传通道**：worker 把 `ExpressionResult(text, level, validation, tts)` 放进**结果队列**，
  心跳每拍 **drain** 一次并落库（`_persist(outcome)`）
- **`_should_speak` 仍在心跳**：触发判断廉价且非阻塞，留在心跳＝「谁决定开口」仍在生命循环里；
  worker 只负责执行

⇒ 口径：**单一事实源＝心跳**（「她说了什么」与「用户说了什么」在同一处落库）。
代价＝落库晚 ≤1 次表达耗时（可接受），换来不必跨循环回传中间态、也不新增崩溃窗口。
...
停收新任务 → 停产生新任务 → 等在跑任务 ≤ `EXPRESS_TIMEOUT_S` → 超时取消 → **排空结果队列并落库** → 停心跳 → 排空写队列 → 关连接。
...
| 1.3 | **心跳接线**（`submit` 非阻塞 + 结果 drain → 落库；此处才是她「不等了」） | **有** |
...
## 二十一、《运行时可靠性收口》落地记录（滚动）

> 设计稿见第二十节；本节按 20.10 拆步**滚动追加**——每步一行交付 / 门禁 / 哈希。
> 目的：交接时一眼看清"哪一步已进真机、哪一步还没有"。**不重复设计理由**（见第二十节）。

### 21.1 步 0.1 基线锚点（2026-09-24）

| 项 | 内容 |
|---|---|
| 交付 | tag `p3-before-runtime-hardening`（指向 `4b97e03`）+ 新增 `docs/BASELINE_2026-09-24.md` |
| 行为变化 | **无**（20.5 规定的阶段 0：纯文档 + tag） |
| 基线实测 | src 52 文件 / 7956 行；tests 33 文件 / 7077 行 / **375 项**；Python 3.12.10；schema 无显式版本号（建表 + 幂等 ALTER 5 条 + 回填 4 条）；heartbeat.db 52.3 MB / heartbeats 120,871 行 / memories 401 行 |
| 已知问题 | **不重写**，引用本节同文件的 **19.4 / 19.5 / 19.7**（避免文档失衡，即 19.4 缺点 8） |
| 门禁 | ruff check ／ ruff format --check ／ mypy strict（52 files）／ pytest 375 passed **全绿** |
| 完整性 | `verify_integrity.py` 清单 **130 → 131 文件**，`--check` **PASS** |
| 提交 | `c69d01c`（1 文件 / +72 −0），已推送 `main`（`origin/main` = `c69d01c`） |
| 装载 | **无需重启**——纯文档 + tag，对运行中的灵魂（PID 12448，启动 19:50:57）零影响 |
