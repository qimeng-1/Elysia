...
### 第十七节 步 A2：成组召回 + D-A6（同一件事被反复提起，更容易浮上来，2026-09-24）
- **触发**：用户拍板主线剩余两项——**D-A3 = 做**（A2 成组召回）、**D-A6 = M9 迁移 / M10 不迁移**（`P3_MEMORY_WORKLOG.md` 第十六节 16.5 的两处同宗判据）。第十六节 16.4 的真实库前后对照已给出 A2 的收益靶子（"被反复提起的那条浮上来"）。**全程未代为重启灵魂**
- **feat（memory/retrieve.py·A2 本体）**：① `MemoryHit.sibling_ids: tuple[int, ...] = ()`（同簇兄弟 id，**代表**是本条；注入文本仍只一行，兄弟只用于加分与观测）；② 簇因子 `_cluster_factor` = `CLUSTER_COEF(0.05) × log(1+兄弟数)`，封顶 `CLUSTER_MAX=0.2`——与 P3-R 复习加成同源（**log 阻尼 + 硬上限**），防"重述条数多"碾压"内容重要"；③ `score_breakdown` / `score_memory` 新增 `cluster_size=0` 参数与 `"cluster"` 分项（保住"`score_breakdown` = `score_memory` 各项之和"的单一事实源）；④ `_dedupe` **去掉"凑满 `max_hooks` 即收工"**、改为**全量归并**并把兄弟 id 记进代表；⑤ `select_hooks` 改**两步打分**：初打分 → 排序 → 全量归并（拿到簇）→ 按 `cluster_size` **重算代表分数并重排** → **再**截取 `max_hooks`
- **落地取舍（未写进设计稿）**：① **截取名额后移**——簇只有在 `_dedupe` 之后才可知，若维持"归并前截取"，"重述多但初次分低"的代表**挤不进名额**、簇加分形同摆设；因代表本就是簇内最高分者，重排只影响**代表之间**的次序；② **`_dedupe` 不再提前收工**——否则排在名额之外的兄弟不计入簇 ⇒ 簇规模系统性偏小、加分与观测都失真（代价：候选集内两两比较不再提前中断，但候选集已过话题门槛、规模本就很小）；③ **两种"簇"口径有意不同**：检索时=候选集内、**分数最高者**为代表；浏览器=只读 SELECT、无话题上下文，**先出现者**为代表，且 docstring 声明"只是观测口径，不参与话语路径"
- **feat（soul/expression_service.py·D-A6/M9）**：`_tool_adopt` 的"候选当代表"（与候选同家的重述不参与并列判定）改用 `same_event(jaccard_threshold=SEDIMENT_CLUSTER_SIMILARITY)`——误判只是"少一条竞争者"，可容忍；**M10 有意不动**：改口作废既有自我认知仍用对称 `content_similarity`（误判会作废她认领过的条目，不可容忍）。docstring 记明"两处**有意不一致**，依据是误判代价不对称"
- **feat（tools/memory_view.py·观测）**：`COLUMNS` 增「**簇**」列（在「召回」与「状态」之间）；明细增"簇"行（`N 条同簇（含自己；检索时计入簇加分）`／`—（没有同簇重述）`）；新增纯函数 `_cluster_sizes(records)`（单趟聚类、判据 `same_event`）。**不加表不加列**
- **真实库只读探针（跑完即删；338 条 / 337 条索引）**：五问（`max_hooks=3`）——「我的生日是哪天来着？」改前 `[145, 81, 105]` → 改后 `[81, 105, 145]`（代表 2 条，兄弟 `#81→[101,103,46]`、`#105→[84,86,34]`）；「你还记得我叫什么名字吗？」`[145, 81, 50]` → `[81, 145, 50]`（`#50→[16,32,18]`）；「我的生日是几月几日？」同第一问；「昨天晚上我们聊了什么？」「花生酱拌饺子好吃吗？」**零簇、零变化**。汇总 **5 问中 3 问排位变化**、代表携带同簇兄弟 **18 条**。**照实说**：①**行数始终 3 条**、未引入无关内容；②变化发生在**同话题、分数档次相近的重述句之间**（正是设计预期），本库语料高度集中在"生日/名字"话题故簇普遍成型，**不代表所有话题都会变**（两问无关话题零变化即反证）；③**两个不同事实仍分属两簇**（`#81`"你的生日" vs `#105`"我的生日"），§15.2 V2 在真实库继续成立
- **测试（+6）**：`test_retrieve.py` +4（簇兄弟记录 / **全量扫描使簇规模诚实**（名额外的兄弟也计入）/ 簇因子加分与封顶 / **簇加分能把代表顶上去**）；`test_expression_service_memory.py` +1（**M9 迁移的真回归锁**——题材选"候选长句 ↔ 换说法短句"使两者 topic 分同为 1.0，**改前必然"没找到"**）；`test_memory_view.py` +1（`_cluster_sizes` 归簇）；另 `test_score_breakdown_includes_importance` 键集断言补 `"cluster"`
- 门禁：ruff lint ✅ / ruff format ✅ / mypy strict ✅ / pytest ✅ **375 passed**（369 + 6）
- **提交**：`c44a3a7`（9 文件 / +334 −36），已推送（`d8ab320..c44a3a7`）；pre-commit 四件套（ruff lint / ruff format check / mypy strict / pytest regression）全部 Passed
- **生效需重启灵魂**（`soul.ps1 stop` → `start -Body`）。**本次不代为重启**，由用户自行决定时机
- **联想网络第二步（第十五节）到此收口**：A1 ✅ → A0 ✅ → A2 ✅ → D-A6 ✅；**embedding（B 案）仍是"按需启动"**。**回退代价极低**：`CLUSTER_COEF` 置 0 即恢复"不带簇加分"的排位（簇列仍可观测）；`sibling_ids` 只增字段、**无 schema 变更**
